...
def load_deepcrack_label_from_xml(img_name, voc12_root):
    from xml.dom import minidom

    el_list = minidom.parse(os.path.join(voc12_root, 'all_xmls', img_name + '.xml')).getElementsByTagName('name')

    multi_cls_lab = np.zeros((2), np.float32)
...
def load_deepcrack_label_list_from_xml(img_name_list, voc12_root):

    return [load_deepcrack_label_from_xml(img_name, voc12_root) for img_name in img_name_list]
...
def load_deepcrack_label_list_from_npy(img_name_list):
    cls_labels_dict = np.load('voc12/cls_deepcrack_labels.npy',allow_pickle=True).item()
    return [cls_labels_dict[img_name] for img_name in img_name_list]

def get_img_path(img_name, voc12_root):
    return os.path.join(voc12_root, IMG_FOLDER_NAME, img_name + '.jpg')

def get_deepcrack_img_path(img_name, voc12_root):
    return os.path.join(voc12_root, deepcrack_folder, img_name + '.png')
...
def load_deepcrack_img_name_list(dataset_path):
...
class DeepcrackImageDataset(Dataset):

    def __init__(self, img_name_list_path, voc12_root, transform=None):
        self.img_name_list = load_deepcrack_img_name_list(img_name_list_path)
        self.voc12_root = voc12_root
        self.transform = transform
    ...
    def __getitem__(self, idx):
        ...
        img = PIL.Image.open(get_deepcrack_img_path(name, self.voc12_root)).convert("RGB")
...
class DeepcrackClsDataset(DeepcrackImageDataset):

    def __init__(self, img_name_list_path, voc12_root, transform=None):
        super().__init__(img_name_list_path, voc12_root, transform)
        self.label_list = load_deepcrack_label_list_from_npy(self.img_name_list)
...
class DeepcrackClsDatasetMSF(DeepcrackClsDataset):
...
class DeepcrackDataset_SelfSup(Dataset):
...
class DeepcrackClsDataset_SelfSup(DeepcrackDataset_SelfSup):
    # path, voc12_root=root, crop=crop, resize=resize, cj=cj
    # path: "voc12/deepcrack/img_train.txt"
    def __init__(self, img_name_list_path, voc12_root, crop, resize, cj):
        super().__init__(img_name_list_path, voc12_root)
    ...
    def apply_tf(self, img, rc=None):
        ...
        # 获得normalize前的img
        img_denorm_rgb = (imutils.denorm(img)) * 255    # tensor: [C,H,W]
        img_denorm_rgb_hwc = img_denorm_rgb.permute(1,2,0) # tensor: [H,W,C]

        img_vgg = self.tf_final(img_vgg)
        img_ori2 = self.tf_final(img_ori2)
        return img,img_vgg,img_ori2, img_denorm_rgb, img_denorm_rgb_hwc

    def __getitem__(self, idx):
        name, img = super().__getitem__(idx)
        # name: IMG33-18_6, 无.png
        # img: 值未变，且shape还是(N,H,W,C)
        label = torch.from_numpy(self.label_list[idx]) # torch([0, 1])
        
        if self.flag_asym_crop:
            img_A,_,_, img_denorm_rgb, img_denorm_rgb_hwc = self.apply_tf(img, rc='A')
        else:
            img_A,img_ori,_ = self.apply_tf(img)
        
        # print("img_A.shape is :{}".format(img_A.shape))
        # # print(img_A)
        # print(torch.max(img_A))
        # print(img_ori.shape)
        # print(torch.max(img_ori))
        # exit()
        ...
        pack['label'] = label
        pack['name'] = name
        pack['img_rgb'] = img_denorm_rgb
        pack['img_rgb_hwc'] = img_denorm_rgb_hwc
