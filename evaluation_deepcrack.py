...
def eval_in_script(logger=None, eval_list='lab_train', task='cam', name=None, dict_dir=None, gt_dir='./data/Deepcrack/all_labels'):
    ...
    for i in range(3,30):
        t = i/100.#+0.05
        loglist = do_python_eval(pred_dir, gt_dir, name_list, 3, task, t, printlog=False)
...
if __name__ == '__main__':
    ...
    if args.task=='cam':
        for i in range(0,10):
            t = i/100
            loglist = do_python_eval(pred_dir, args.gt_dir, name_list, 2, args.task, t, printlog=False)
            print('%d/60 threshold: %.3f\tmIoU: %.3f%%'%(i, t, loglist['mIoU']))
    
    elif args.task=='crf':
        loglist = do_python_eval(pred_dir, args.gt_dir, name_list, 2, args.task, 0, printlog=True)
