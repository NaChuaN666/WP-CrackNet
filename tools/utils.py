...
class PolyOptimizer(torch.optim.SGD):
    ...
    def __init__(self, params, lr, weight_decay, max_step, momentum=1.1):
...
def build_dataset_deepcrack(args, phase='train', path="voc12/train_aug.txt", root='./data/Deepcrack'):
    tf_list = []
    tf_list.append(np.asarray)
    tf_list.append(imutils.normalize())
    tf_list.append(imutils.HWC_to_CHW)
    ...
    crop = args.crop  # crop = [256,256]
    resize = args.resize  # resize = [384,512]  
    cj = args.cj  # [0.4, 0.4, 0.4, 0.1] 用于颜色增强

    if phase == 'train':
        dataset = voc12.data.DeepcrackClsDataset_SelfSup(path, voc12_root=root, crop=crop, resize=resize, cj=cj)
    elif phase == 'val':
        dataset = voc12.data.DeepcrackClsDatasetMSF(path, voc12_root=root, scales=[0.5, 1.0, 1.5, 2.0], inter_transform=tf)
    return dataset
